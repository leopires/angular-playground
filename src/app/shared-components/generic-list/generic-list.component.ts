...
export class GenericListComponent implements OnInit, OnDestroy {
  ...
  @Output()
  selectedItem: EventEmitter<GenericListItem> =
    new EventEmitter<GenericListItem>();

  constructor(
    private genericListDataFetcherService: AbstractGenericListDataFetcher
  ) {
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.observable.subscribe(observer => {
      this.data = observer;
      console.log(
        `[${getClassName(this)}]: Current Component Data -> `,
        this.data
      );
    });
  }
